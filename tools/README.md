...
## Loot icons (the game's real item icons)

The icons in `quests/zones/_loot-icons/*.png` are the game's own item icons —
most generated by its procedural `iconDataUrl`, a few weapons rasterized from
hand-painted `.jpg` thumbnails under `public/ui/weapons`.

```sh
npx tsx build-loot-manifest.ts loot-items.json            # item ids that drop in the bestiary
node render-loot.mjs loot-items.json <woc>/public ../quests/zones/_loot-icons
```

`render-loot.mjs` bundles the game's `src/ui/icons.ts` (via esbuild) into a
browser script, runs it in headless Chromium, and saves each item's icon. Needs
`npm i esbuild playwright`. Also run **after** generate-quests.ts.
