    ...
    if(!p.dead){
      ...
      if(cmd.cmd==='kill'&&cmd.id!=null){
        ...
        if(!t||t.dead){status='target_dead';}
          ...
          if(dist<=8){try{s.castAbility&&s.castAbility('frost_nova');s.castAbility&&s.castAbility('ice_barrier');}catch{}} // CC+shield when meleed (session-08 lesson)
      ...
      else if(cmd.cmd==='idle'){try{['frost_armor','arcane_intellect'].forEach(a=>s.castAbility&&s.castAbility(a));}catch{}status='buff+regen';}
