# Idols of the Deep

> Quest ID: `q_idols` · Zone 2 — Mirefen Marsh

| | |
|---|---|
| **Recommended level** | 6+ (zone range 6–13) |
| **Quest giver** | **Brother Aldric**, Priest of the Vale _(at ~x:-15, z:296)_ |
| **Turn in to** | **Brother Aldric**, Priest of the Vale _(at ~x:-15, z:296)_ |
| **Requires** | The Deepfen Stirs (`q_deepfen`) |

## Story

> Fenwick's wardens say the fish-men dredge idols from the lake bottom and clutch them like holy relics. If those idols are what I fear, I must see them with my own eyes. Take 5 from the Deepfen snappers — they will not part with them kindly.

## How to complete

- **Collect 5× Waterlogged Idol**
  - Drops from [**Deepfen Snapper**](bestiary.md#mob-deepfen_murloc) (50% chance) — Found in the open world at ~x:-82, z:273 (8 mobs, radius 15); Found in the open world at ~x:-120, z:350 (6 mobs, radius 13)
  - _Tracker: Waterlogged Idol_

Then return to **Brother Aldric**, Priest of the Vale _(at ~x:-15, z:296)_ to turn in.

## Rewards

- **XP:** 1050
- **Money:** 400 copper

## On completion

> Gravecaller work — older than Morthen, older than me. The sect did not begin in Eastbrook, $N. It began here, and the lake has been keeping its secrets.

## Leads to

- Back to the Shallows (`q_deepfen_purge`)

## Where to go

![Where to go for Idols of the Deep](qmap-q-idols.svg)

**[🧭 Open this route in 3D →](#/questroute/q_idols)**

_Numbered route: ① start → objectives → 3 turn in. Faint dots are the rest of the zone for context — see the [full zone map](README.md). Mob names above link to the [bestiary](bestiary.md)._
