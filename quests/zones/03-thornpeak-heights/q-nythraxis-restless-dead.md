# Unrest in the Bonefields

> Quest ID: `q_nythraxis_restless_dead` · Zone 3 — Thornpeak Heights

| | |
|---|---|
| **Recommended level** | 20+ |
| **Quest giver** | **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ |
| **Turn in to** | **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ |

## Story

> Something has changed in Thornpeak Heights, <your name>. The dead no longer wander aimlessly. They gather and march through the northern bonefields beyond Highwatch, where the old battlefield meets the cliff road. Go there, investigate the unrest among the Boneclad Revenants, and bring back any proof of what is driving them.

## How to complete

- **Collect 10× Runed Bone Shard**
  - Drops from **Boneclad Revenant** (70% chance) — Found in the open world at ~x:-40, z:830 (8 mobs, radius 20); Found in the open world at ~x:-15, z:860 (6 mobs, radius 16)
  - _Tracker: Runed Bone Shard_

Then return to **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ to turn in.

## Rewards

- **XP:** 4200
- **Money:** 2000 copper

## On completion

> The same mark appears on every shard... a crown. I have seen this before, cut into old graves no Eastbrook record remembers.

## Leads to

- Graves of the Forgotten (`q_nythraxis_graves`)

## Zone map

![Map of Zone 3 — Thornpeak Heights](map.svg)

_Gold = NPCs · red = mob camps · purple = dungeons · green = ground pickups. Match the names above to the markers._

See the **[zone bestiary](bestiary.md)** for the health, armor, and kill tactics of every mob named above.
