# Graves of the Forgotten

> Quest ID: `q_nythraxis_graves` · Zone 3 — Thornpeak Heights

| | |
|---|---|
| **Recommended level** | 20+ |
| **Quest giver** | **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ |
| **Turn in to** | **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ |
| **Requires** | Unrest in the Bonefields (`q_nythraxis_restless_dead`) |

## Story

> I have seen these marks before, on three old graves around the northern battlefield. Captain Aldren lies on the eastern rise, High Priest Malric near the central broken road, and Royal Assassin Voss by the western cliff. Touch each grave and listen, <your name>. The dead may remember what the living forgot.

## How to complete

- **Interact with Grave of Captain Aldren**
  - Locations: ~x:138, z:838
  - _Tracker: Vision at the Grave of Captain Aldren_
- **Interact with Grave of High Priest Malric**
  - Locations: ~x:141, z:712
  - _Tracker: Vision at the Grave of High Priest Malric_
- **Interact with Grave of Royal Assassin Voss**
  - Locations: ~x:-139, z:787
  - _Tracker: Vision at the Grave of Royal Assassin Voss_

Then return to **Brother Aldric**, Priest of the Vale _(at ~x:-10, z:656)_ to turn in.

## Rewards

- **XP:** 4300
- **Money:** 2200 copper

## On completion

> Aldren remained loyal, Malric refused to accept death, and Voss saw the danger before anyone else. All three served the same forgotten king.

## Leads to

- The Abandoned Crypt (`q_nythraxis_sealed_crypt`)

## Where to go

![Where to go for Graves of the Forgotten](qmap-q-nythraxis-graves.svg)

**[🧭 Open this route in 3D →](#/questroute/q_nythraxis_graves)**

_Numbered route: ① start → objectives → 5 turn in. Faint dots are the rest of the zone for context — see the [full zone map](README.md). Mob names above link to the [bestiary](bestiary.md)._
