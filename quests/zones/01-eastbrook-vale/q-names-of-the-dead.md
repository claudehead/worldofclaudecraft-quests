# The Names of the Dead

> Quest ID: `q_names_of_the_dead` · Zone 1 — Eastbrook Vale

| | |
|---|---|
| **Recommended level** | 1+ (zone range 1–7) |
| **Quest giver** | **Brother Aldric**, Priest of the Vale _(at ~x:-14, z:-10)_ |
| **Turn in to** | **Brother Aldric**, Priest of the Vale _(at ~x:-14, z:-10)_ |
| **Requires** | Whispers Below (`q_whispers`) |

## Story

> If the Gravecallers raised our dead, I must know whose graves they robbed. The chapel sexton kept a burial ledger, and the wind has scattered its pages across the chapel yard. Gather 3 of them for me, <your name> — the dead deserve to be called by their names.

## How to complete

- **Collect 3× Weathered Ledger Page**
  - Pick up from the ground (sparkle objects) at: ~x:78, z:84 · ~x:83, z:88 · ~x:86, z:92
  - _Tracker: Weathered Ledger Page_

Then return to **Brother Aldric**, Priest of the Vale _(at ~x:-14, z:-10)_ to turn in.

## Rewards

- **XP:** 600
- **Money:** 250 copper

## On completion

> These poor souls... and look here. Sexton Marrow — the chapel's own living caretaker — his grave the first disturbed. Morthen began with the very man who buried Eastbrook's dead.

## Leads to

- Silence the Call (`q_silence_the_call`)

## Zone map

![Map of Zone 1 — Eastbrook Vale](map.svg)

_Gold = NPCs · red = mob camps · purple = dungeons · green = ground pickups. Match the names above to the markers._
